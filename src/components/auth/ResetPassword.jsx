...
import { useRequestResetPasswordMutation } from "../redux/user/userApiSlice";
...
const ResetPassword = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await requestReset(formData).unwrap;
      navigate("/link");
    } catch (err) {
      ...
      <div className="col-span-2 bg-[#f4f4f4] px-12 mx-auto pt-32 rounded-[50px] border border-[#cbceda] h-full">
        ...
        <div className="w-full text-gray-600 pt-16 pb-12">
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-5 ">
            ...
            {error && (
              <p className="text-[#D61B1B]">
                Looks like you do not have an account with us. If you need an
                account, Sign up. If you believe this ia and error, please
            ...
            <div className="text-start pt-12">
              <Link to="/personal-info" className="">
                Need an Account?
                <span className="text-[#9cabd7] hover:text-indigo-600">
                  Sign up
                </span>
              </Link>
            </div>
